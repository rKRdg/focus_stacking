...
def align_image(baseimage,images):
	...
	kp1, des1 = detector.detectAndCompute(baseimage, None)
	...
	for imgi in range(len(images)):
		...
		images[imgi]=cv2.warpPerspective(image2, np.linalg.inv(M), (image2.shape[1], image2.shape[0]))
...
def focus_stack(baseimage, images):
	print("start focus stacking")

	if config.HIST_MATCHING:
		img_yuv0 = cv2.cvtColor(baseimage, cv2.COLOR_BGR2YUV)
		...
		for imgi in range(len(images)):
			...
			images[imgi] = cv2.cvtColor(img_yuv1, cv2.COLOR_YUV2BGR)
	...
	for image2 in images:
		...
		image1=(output/output_count).astype(np.uint8)
...
images=align_image(baseimage,images)


result=focus_stack(baseimage,images)
if config.DENOISE:
